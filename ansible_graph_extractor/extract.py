...
def extract_one(args: tuple[str, str, Path], log_reset: bool = True) -> SuccessResult | FailResult:
    log_stream = StringIO()
    if log_reset:
        logger.remove()
    ...
    try:
        ...
        return role_id, neo4j_str, graphml_str, dot_str, visibility_dump, error_str, log_str
...
def tmp_copy_repo(input_path: Path) -> Iterator[Path]:
    with tempfile.TemporaryDirectory() as tmpd:
        shutil.copytree(input_path, tmpd, dirs_exist_ok=True, symlinks=True, ignore_dangling_symlinks=True)
        yield Path(tmpd)


CommitInfo = tuple[str, str, list[str]]  # SHA1, message, parents. Rest of the info should be in the dataset already
...
def iter_commits_and_checkout(repo_path: Path) -> Iterable[CommitInfo]:
    ...
    for commit in queue:
        ...
        queue.extend(parent for parent in commit.parents if parent.hexsha not in seen_shas)
...
def extract_full_repo(args: tuple[str, Path, Path]) -> tuple[str, list[str], list[str], list[CommitInfo]]:
    try:
...
def _extract_full_repo(args: tuple[str, Path, Path]) -> tuple[str, list[str], list[str], list[CommitInfo]]:
    ...
    outs = {subdir: (output_path / subdir / role_id) for subdir in ('graphml', 'errors', 'aux')}
    ...
    with tmp_copy_repo(role_path) as repo_path:
        ...
        for commit_info in iter_commits_and_checkout(repo_path):
            ...
            _, neo4j_str, graphml_str, dot_str, vis_str, error_str, log_str = cast(SuccessResult, result)
    ...
    assert len(exceptions) + len(success_paths) == len(commit_infos), 'Missing commits?!'
...
def load_from_previous_run(role_id: str, output_path: Path) -> tuple[str, list[str], list[str], list[CommitInfo]]:
    commit_infos = json.loads((output_path / 'status' / (role_id + '.json')).read_text())
    all_graph_xmls = [f for f in (output_path / 'graphml' / role_id).glob('**/*.xml') if f.is_file()]
    all_graph_shas = {f.stem for f in all_graph_xmls}
    assert len(all_graph_shas) == len(all_graph_xmls)
    all_errors = [f for f in (output_path / 'errors' / role_id).glob('**/*.txt') if f.is_file()]
    ...
    if not len(fatal_error_shas | all_graph_shas) == len(commit_infos):
        print(f'Missing commits?! {len(fatal_error_shas | all_graph_shas)} vs {len(commit_infos)}')

    errors = [load_error(role_id, output_path, error_sha) for error_sha in fatal_error_shas]
...
def load_error(role_id: str, output_path: Path, error_sha: str) -> str:
    error_path = (output_path / 'errors' / role_id).joinpath(*error_sha[:3]) / (error_sha + '.txt')
