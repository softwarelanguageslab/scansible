...
@click.option('-q', '--quiet', is_flag=True, default=False, help='Print only warnings and errors')
def cli(verbose: bool, quiet: bool) -> None:
    """Static Code Analysis for Ansible."""
    if verbose and quiet:
        raise click.BadOptionUsage('verbose', '--verbose and --quiet are mutually exclusive')
    # Set up logging
    logger.remove()
...
@click.option('-o', '--output', type=click.File(mode='wt'), default='-', help='File to write output to, defaults to stdout')
@click.option('-f', '--format', 'output_format', type=click.Choice(['graphml', 'neo4j', 'dot']), default='graphml', help='Output format (default: graphml)')
...
@click.option('--version', type=str, default='latest', help='Project version (default: "latest"")')
@click.option('--aux', 'aux_file', type=click.File(mode='wt'), help='File to write auxiliary information to (default: not written)')
@click.option('--errors', 'errors_file', type=click.File(mode='wt'), help='File to write error log to (default: not written)')
...
def build_pdg(
    ...
    role_search_paths = list(role_search_path) + [Path(p) for p in ans_constants.DEFAULT_ROLES_PATH]
...
def check(
    ...
    ctx = extract_pdg(project_path, name, 'latest', role_search_paths, as_pb=as_pb, lenient=not strict)
    pdg = ctx.graph
