...
from .utils import get_nodes, get_def_expression, determine_value_version_change_reason, ValueChangeReason, find_variable_usages, get_all_used_variables, get_def_conditions, get_register_all_used_variables, is_registered_variable, register_task_has_conditions
    ...
    def scan_vars(self, graph: Graph, name: str, nodes: dict[int, list[Variable]], visinfo: VisibilityInformation) -> Iterable[RuleResult]:
        ...
        for _, vals_v2_lst in nodes_sorted:
            ...
            all_cond_uses_v1 = set(
                    cu1 for cond_v1 in get_def_conditions(graph, v1)
                    for cu1 in get_all_used_variables(graph, cond_v1))
            all_cond_uses_v2 = set(
                    cu2 for cond_v2 in get_def_conditions(graph, v2)
            ...
            v1_ever_used = any(find_variable_usages(graph, v1_val) for v1_val in vals_v1)
            is_due_to_register = (
                    is_registered_variable(graph, v2)
            ...
            if not v1_ever_used:
                warning_expl_lines.append('Due to this override, the original definition is never used.')
            if is_due_to_register:
                warning_expl_lines.append(f'{v2!r} is defined through `register` on a conditionally executed task. However, even when this task is skipped, the variable is still defined.')
            if e2 is not None:
                warning_expl_lines.append(f'Initialiser of {v2!r}: {e2.expr!r}')
            if all_cond_uses_v1:
                vstr = ', '.join(uv1.name for uv1 in all_cond_uses_v1)
                warning_expl_lines.append(f'Variables used in conditional definition of original definition ({v1!r}): {vstr}')
            if all_cond_uses_v2:
                vstr = ', '.join(uv2.name for uv2 in all_cond_uses_v2)
                warning_expl_lines.append(f'Variables used in conditional definition of new definition ({v2!r}): {vstr}')
