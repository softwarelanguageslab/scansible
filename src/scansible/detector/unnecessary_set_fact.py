...
def get_var_origin(graph: Graph, node: Variable) -> Expression | Literal | Task | None:
    ...
    if def_literal:
        assert len(def_literal) == 1, f'Expected {node!r} to be defined by one literal, found {len(def_literal)}'
