...
def extract_variable_name(warning: list[str]) -> str:
    ...
    if rule == "Unconditional override":
        mtch = re.match(
            r'Potential unintended unconditional override of variable "([^@]+)@\d+"',
            title,
        )
    ...
    if rule == "Unused because shadowed":
        mtch = re.match(
            r'Unused variable "([^@]+)@\d+" because it is already defined', title
        )
        assert mtch is not None, title
        return mtch.group(1)
    ...
    if cat == "Unnecessarily high precedence":
        mtch = re.match(
            r'Unnecessary use of (?:set_fact|include_vars) for variable "([^@]+)@\d+"',
            title,
        )
        if mtch is None:
            print(title)
    ...
    if cat == "Unsafe reuse":
        mtch = re.match(
            r'Potentially unsafe reuse of variable "([^@]+)@\d+" due to', title
        )
        assert mtch is not None, title
        return mtch.group(1)
...
def find_file_path_changes(
    role_path: Path, paths: set[str]
) -> tuple[AddedFiles, DeletedFiles, RenamedFiles]:
    ...
    for commit in commit_it:
        ...
        for parent in commit.parents or [git.NULL_TREE]:
            ...
            for diff in commit.diff(
                parent, paths=list(paths), R=parent is not git.NULL_TREE
            ):
...
]  # role_id, commit_sha, commit_msg, committed_datetime, committer, author
CommitTag = tuple[str, str, str]  # role_id, commit_sha, tag


def extract_commit_info(
    role_meta_path: Path, prev_commits: list[tuple[str, str, str]]
) -> tuple[list[ExtCommit], list[CommitTag]]:
    ...
    for c in prev_commits:
        ...
        ext_commits.append(
            (
                *c,
                cmeta["committed_datetime"],
                cmeta["committer_name"],
                cmeta["author_name"],
            )
        )
...
def extract_all_locations(
    ...
    try:
        ...
        locs = {
            extract_file_path(n.location)
            for n in graph
            if hasattr(n, "location") and n.location != "external file"
        }
