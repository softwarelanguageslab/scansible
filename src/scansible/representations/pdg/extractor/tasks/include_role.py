...
class IncludeRoleExtractor(TaskExtractor):
    def extract_task(self, predecessors: Sequence[rep.ControlNode]) -> ExtractionResult:
        with self.setup_task_vars_scope(ScopeLevel.INCLUDE_PARAMS):
            ...
            with self.context.include_ctx.load_and_enter_role(
                ...
                role_result = RoleExtractor(self.context, incl_role).extract_role(
                    predecessors
                )
            ...
            return role_result.add_control_nodes(
                condition_result.added_control_nodes
            ).add_next_predecessors(condition_result.added_control_nodes)
