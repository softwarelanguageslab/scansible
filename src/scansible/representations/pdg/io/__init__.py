...
def dump_graph(output_format: str, graph: Graph) -> str:
    match output_format:
        case 'graphml': dumper = graphml_dump
        case 'neo4j': dumper = neo4j_dump
        case 'graphviz': dumper = lambda g: dot_dump(g).source
        case _: raise ValueError(f'Unknown output format: {output_format}')
