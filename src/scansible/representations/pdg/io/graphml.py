...
class CustomGraphMLWriter(WriterBase):
    def _add_edge_attributes(
        self, scope: str, xml_obj: Element, edge: rep.Edge, default: dict[str, object]
    ) -> None:
    ...
    def add_edges(
        ...
        for u, v, key, data in g.edges(data=True, keys=True):
            edge_element = self.myElement(
                ...
                target=str(v.node_id),
                id=f"{u.node_id}-{v.node_id}-{key}",
            )
...
class CustomGraphMLReader(ReaderBase):
    def __init__(
        self,
        node_type: Type[rep.Node] = rep.Node,
        edge_key_type: Type[int] = int,
