...
from attrs_strict import type_validator as old_type_validator
...
def _convert_union_type(type_: Any) -> object:
    ...
    if type_.__origin__ in (list, dict):
        return types.GenericAlias(
            type_.__origin__, tuple(_convert_union_type(arg) for arg in type_.__args__)
        )
    return type_
...
def type_validator(
    empty_ok: bool = True,
) -> Callable[[Any, attrs.Attribute[Any], Any], None]:
    ...
    def converting_validator(
        instance: Any, attribute: attrs.Attribute[Any], field: Any
    ) -> None:
        try:
            ...
            if (
                not e.__context__
                or e.__context__.__class__.__name__ != "_StringAnnotationError"
            ):
                raise
            convert_types(attribute)
            old_validator(instance, attribute, field)
