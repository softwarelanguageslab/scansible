...
def describe_dump_node() -> None:
    ...
    def should_dump_expression(g: Graph) -> None:
        e = Expression(expr='{{ test }}', node_id=0)

        result = dump_node(e, g)

        assert result == 'CREATE (n0:Expression { expr: "{{ test }}", node_id: 0, role_name: "testrole", role_version: "v1.0.0" })'

    def should_dump_variable(g: Graph) -> None:
        v = Variable(name='test', version=1, node_id=0)

        result = dump_node(v, g)

        assert result == 'CREATE (n0:Variable { name: "test", node_id: 0, role_name: "testrole", role_version: "v1.0.0", version: 1 })'

    def should_dump_task(g: Graph) -> None:
        t = Task(action='file', name='task name', node_id=0)

        result = dump_node(t, g)

        assert result == 'CREATE (n0:Task { action: "file", name: "task name", node_id: 0, role_name: "testrole", role_version: "v1.0.0" })'

    def should_dump_literal_string(g: Graph) -> None:
        l = Literal(type='str', value='literal value', node_id=0)

        result = dump_node(l, g)

        assert result == 'CREATE (n0:Literal { node_id: 0, role_name: "testrole", role_version: "v1.0.0", type: "str", value: "literal value" })'

    @pytest.mark.parametrize('type, value', [
            ('int', 0), ('int', 10), ('int', -10),
            ('float', 0.0), ('float', 0.10), ('float', -0.10),
            ('bool', True), ('bool', False)])
    def should_dump_literal_non_string(g: Graph, type: str, value: Any) -> None:
        l = Literal(type=type, value=value, node_id=0)

        result = dump_node(l, g)

        assert result == f'CREATE (n0:Literal {{ node_id: 0, role_name: "testrole", role_version: "v1.0.0", type: "{type}", value: {value} }})'
...
def describe_dump_edge() -> None:
    ...
    def should_dump_order_edge() -> None:
        ...
        assert result == 'CREATE (n1)-[:ORDER { transitive: False }]->(n2)'
...
def describe_dump_graph() -> None:
    ...
    def should_return_query_for_graph_with_multiple_nodes_without_edges(g: Graph) -> None:
        t = Task(action='file', name='task name', node_id=0)
        v = Variable(name='avar', version=1, node_id=1)
        g.add_node(t)
        g.add_node(v)
