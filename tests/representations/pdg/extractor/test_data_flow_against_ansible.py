...
_ansible_var_names = st.sampled_from(
    ["a", "b", "c"]
)  # Only allowing a limited number here because we WANT overlaps, that's the whole point
...
class Block:
    ...
    def depends_on(
        self, name: str, dep_name: str | set[str], real_dep_name: str | None = None
    ) -> bool:
...
class CodeGen:
    ...
    def _add_block_local(self) -> None:
        ...
        block_vars = cast(
            dict[str, object], self.parent_block.content[-1].setdefault("vars", {})
        )
        block_vars[name] = val
    ...
    def _add_task_local(self) -> None:
        ...
        task_vars = cast(
            dict[str, object], self.curr_block.content[-1].setdefault("vars", {})
        )
        task_vars[name] = val
        self._used_variables.add(name)
    ...
    def _add_include(self) -> None:
        file_name = self.draw(
            _ansible_file_names.filter(lambda n: n not in self._included_file_names)
        )
        ...
        incl_task: dict[str, object] = {"include_vars": str(file_path)}
    ...
    def _add_set_fact(self) -> None:
        ...
        if should_reuse:
            reuse_var = self.draw(
                st.sampled_from(sorted(self.curr_block.all_var_deps.keys()))
            )
            val = f"{{{{ {reuse_var} }}}}"
        else:
            val = self._next_value

        task: dict[str, object] = {"set_fact": {name: val}}
    ...
    def _add_include_tasks(self) -> None:
        file_name = self.draw(
            _ansible_file_names.filter(lambda n: n not in self._included_tasks_names)
        )
        ...
        if not def_at_runtime:
            ...
            for reusable_var in reusable_vars:
                if reusable_var in self._set_facts_vars:
                    ...
                    continue
                if self._include_vars_stack and self._include_vars_stack[-1].depends_on(
                    reusable_var, name
                ):
    ...
    def _add_print(
        ...
        task: dict[str, object] = {
            "debug": {"msg": f"{self.last_prefix} is {{{{ {name} }}}}"}
        }
        self.last_prefix += 1
        if prepend:
